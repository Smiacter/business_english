...
class AuthInterceptor extends Interceptor {
  ...
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) {
    String accessToken = SpUtil.getString(NetConfig.accessToken) ?? "";
...
class TokenInterceptor extends Interceptor {
  Future<String> getToken() async {
    ...
    params['refresh_token'] = SpUtil.getString(NetConfig.refreshToken) ?? "";
    try {
      _tokenDio.options = NetManager.instance.dio.options;
  ...
  onResponse(Response response, ResponseInterceptorHandler handler) async {
    ...
    //   Log.d('-----------自动刷新Token------------');
    //   Dio dio = NetManager.instance.dio;
    //   dio.interceptors.requestLock.lock();
    //   String accessToken = await getToken(); // 获取新的accessToken
    //   Log.e('-----------NewToken: $accessToken ------------');
    //   SpUtil.putString(NetConfig.accessToken, accessToken);
